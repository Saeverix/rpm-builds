...
| `noctalia-greeter` | 1.3.1 | `noctalia-greeter` | fc44 | <https://github.com/noctalia-dev/noctalia-greeter> |
...
`noctalia-greeter` is the greetd login greeter that matches Noctalia Shell's look.
Unlike `noctalia`, upstream creates no GitHub Releases for this repo -- only tags
(`git tag v1.3.1` etc, confirmed by an empty response from the releases API) -- so
`Source0` uses the autogenerated tag archive, the same pattern `mangowm` and
`scenefx` use, rather than an uploaded release asset like `ghostty`.
...
Tagging this package needed a fix to `build-noctalia.yml` first: its tag trigger
was a plain `noctalia-*` prefix glob, which also matches `noctalia-greeter-*`, so
tagging a greeter release would have spuriously re-triggered the unrelated
`noctalia` build. Its filter is narrowed to `noctalia-[0-9]*`, which still matches
`noctalia-5.0.0-beta.10-1` but not `noctalia-greeter-1.3.1-1`.

`PACKAGING.md` asks packagers to `chown /var/lib/noctalia-greeter` to the greetd
session user in `%post`, since the greeter's own tools resolve that user from the
state directory's ownership before ever trying to parse greetd's config. It also
ships an *optional* `tmpfiles.d` drop-in for that same directory, but hardcodes it
to `greeter:greeter`. Fedora's own `greetd` package creates a `greetd` system user
via `sysusers.d` instead, and its default `/etc/greetd/config.toml` runs the
session as `greetd` -- not `greeter`. So the spec deletes the shipped drop-in in
`%install` and does the chown to `greetd` itself in `%post`, exactly the fallback
`PACKAGING.md` describes for a nonstandard session user.
